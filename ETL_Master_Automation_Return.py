...
# ── FLIPKART ───────────────────────────────────────────────────
FK_PDF_ROOT   = r"C:\Users\Dilip\OneDrive - Happy Ecom Venture\Desktop\FK"
FK_MASTER_XLS = r"C:\Users\Dilip\OneDrive - Happy Ecom Venture\Desktop\FK_POD_Master.xlsx"
...
# MYNTRA_INPUT  = r"C:\Users\Dilip\OneDrive - Happy Ecom Venture\Desktop\Myntra.xlsx"
...
def build_fk_order_lookup() -> dict:
    ...
    Excel formula jo Python mein convert ki:
      =XLOOKUP(TrackingID, AN:AN, E:E,,0)

    Column mapping (RETURN EN sheet):
      AN (index 39) = Reverse Tracking No  ← hamare Tracking_ID se match
      N  (index 13) = Tracking No           ← fallback agar AN mein nahi mila
      E  (index 4)  = Magento Order No      ← RIGHT(18) karke "OI:" prefix

   Returns: {tracking_id: Magento_Order_No}
    """
    if not os.path.exists(FK_ORDER_MIS):
        print(f"  ⚠️ FK Order MIS file nahi mili: {FK_ORDER_MIS}")
        print(f"     Order_ID column 'Not Found' rahega.")
        return {}

    print(f"  📂 FK Order MIS load ho rahi hai: {Path(FK_ORDER_MIS).name}")
    try:
        df = pd.read_excel(FK_ORDER_MIS, sheet_name="RETURN EN", engine="openpyxl", dtype=str)
    except Exception as e:
        print(f"  ⚠️ FK Order MIS load error: {e}")
        return {}

    if df.shape[1] < 40:
        print(f"  ⚠️ RETURN EN sheet mein expected 40+ columns nahi milein (mila: {df.shape[1]})")
        return {}

    an_col = df.iloc[:, 39].fillna("")   # AN = Reverse Tracking No
    n_col  = df.iloc[:, 13].fillna("")   # N  = Tracking No (fallback)
    e_col  = df.iloc[:, 4].fillna("")    # E  = Magento Order No

    lookup = {}
    for an_val, n_val, e_val in zip(an_col, n_col, e_col):
       e_str = str(e_val).strip()
if not e_str or e_str.upper() == "NAN":
    continue

order_id = e_str

        # AN column primary (same as Excel formula)
        an_clean = re.sub(r'\s+', '', str(an_val)).strip().upper()
        if an_clean and an_clean not in lookup:
            lookup[an_clean] = order_id

        # N column fallback
        n_clean = re.sub(r'\s+', '', str(n_val)).strip().upper()
